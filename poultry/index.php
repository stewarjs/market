...
<?php
	if(!isset($house_id)){
		...
		echo '</div><aside class="flex__box--20 aside--right"><select data-update="false" data-house="'. $house_id .'" id="bird_options" aria-label="Select an option"><option selected disabled value="">Select an Option</option><option id="move_bird" value="move_bird">Move Bird(s)</option><option id="delete_bird" value=delete_bird"">Delete Bird(s)</option><option id="harvest_bird" value="harvest_bird">Harvest Bird(s)</option><option id="log_eggs" value="log_eggs">Log Egg Production</option></select><h4 class="heading--label">House Stats</h4><ul class="stats"><li><span class="stats__value">' . round(total_eggs($house_id), 1) . '</span><span class="stats__label">Total Eggs</span></li><li><span class="stats__value">$' . round(total_cost($house_id), 2) . '</span><span class="stats__label">Total Costs</span></li></ul></aside></div>';
	}
